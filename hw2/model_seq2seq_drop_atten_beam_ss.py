...
def build_model(n_words, bias_init_vector=None):
    ...
    encoder_cell = tf.nn.rnn_cell.DropoutWrapper(encoder_cell, output_keep_prob=dropout_prob)
    ...
    helper = tf.contrib.seq2seq.ScheduledEmbeddingTrainingHelper(decoder_emb_inp, decoder_seq_length, embedding_decoder, sample_rate, time_major=False)
...
def build_generator(n_words, bias_init_vector=None):
    ...
    attention_mechanism = tf.contrib.seq2seq.LuongAttention( dim_hidden, tiled_encoder_outputs)
    ...
    decoder = tf.contrib.seq2seq.BeamSearchDecoder(decoder_cell, embedding_decoder, tf.fill([batch_size], 1), 2,
        decoder_cell.zero_state( batch_size * beam_width, tf.float32).clone(cell_state=tiled_encoder_final_state), beam_width, output_layer = projection_layer)

    # Dynamic decoding
    outputs, _, _ = tf.contrib.seq2seq.dynamic_decode(decoder,  maximum_iterations=caption_lstm_step)

    #generated_words = outputs.predicted_ids
    generated_words = outputs
...
def test(model_path='./models/model-200'):
    ...
    for idx, video_feat_path in enumerate(test_videos):
        ...
        ids=[]
        max_id = np.argmax(probs_val.beam_search_decoder_output.scores[-1])

        for id in probs_val.predicted_ids[0]:
            ids.append(id[max_id])

        generated_words = ixtoword[ids]
