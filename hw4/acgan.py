...
import time
import os
from libs.ops import get_perturbed_batch
from network.model_srresnet import Generator_srresnet, Discriminator_srresnet, Discriminator_srresnet_sn
...
class ACGAN(object):
    ...
    def build_model(self):
        ...
        #gradient penalty
        alpha = tf.random_normal(shape=self.img.get_shape())
        interpolates = alpha * self.img + (1-alpha) * self.img_p
        pred_hat, d_cls=self.d_net(self.seq, interpolates, update_collection = "NO_OPS")
        gradients = tf.gradients(pred_hat, [interpolates])[0]
    ...
    def train(self):
        ...
        for t in range(self.FLAGS.iter):
            ...
            feed_dict = {
                ...
                self.img_p: get_perturbed_batch(img),
                self.z:z,
            }
