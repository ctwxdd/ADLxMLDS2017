...
import time
import os
#from model_ import Generator, Discriminator, Discriminator_wgan_gp
#from model_sn import Generator_resnet, Discriminator_sn, Discriminator_sn_resnet
import util
...
class DCGAN(object):
    ...
    def build_model(self):
        ...
        self.d_net = Discriminator_sn_resnet( 
                        embedding_size=self.FLAGS.embedding_dim, 
                        hidden_size=self.FLAGS.hidden,
                        img_row=self.img_row,
                        img_col=self.img_col)
        ...
        self.w_img = tf.placeholder(tf.float32, [64, self.img_row, self.img_col, 3], name="w_img")
        ...
        self.d = self.d_net(r_seq, r_img, reuse=False)     # r img, r text
        self.d_1 = self.d_net(r_seq, self.f_img)         # f img, r text
        self.d_2 = self.d_net(self.w_seq, self.w_img)        # r img, w text
        ...
        self.d_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.d, labels=tf.ones_like(self.d))) \
            + (tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.d_1, labels=tf.zeros_like(self.d_1))) + \
               tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.d_2, labels=tf.zeros_like(self.d_2)))) / 2
