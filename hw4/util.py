...
def preprocessing(preproc_dir, img_dir, tag_path, eye_map, hair_map):
    ...
    with open(tag_path, 'r') as f:
        for idx, row in enumerate(csv.reader(f)):
            ...
            feat_p5 = misc.imresize(feat_p5, [75, 75, 3])
            feat_p5 = crop_center(feat_p5, 64,64)

            attrib_tags.append([hair_idx, eyes_idx])
            img_feat.append(feat_p5)

            feat_m5 = misc.imrotate(feat, -5)
            feat_m5 = misc.imresize(feat_m5, [75, 75, 3])
            feat_m5 = crop_center(feat_m5, 64,64)
...
class Data(object):
    def __init__(self, img_feat, tags, z_dim):
        ...
        #o b, a p, g y, g g
        self.test = [[2,3], [3,4], [5,6]]
        self.test_tags_idx = self.gen_test_hot()
        self.curr_w_idx = 0


        self.fixed_z = self.next_noise_batch(len(self.test), z_dim)
    ...
    def gen_test_hot(self):
        ...
        for tag in self.test:
            test_hot.append(self.make_one_hot(tag[0], tag[1]))

        return np.array(test_hot)

    def load_test(self,test_path, hair_map, eye_map):
        ...
        with open(test_path, 'r') as f:
            ...
            for line in f.readlines():
                ...
                test.append(self.make_one_hot(hair, eye))
       
    
        self.test_tags_idx = test


    def make_one_hot(self, hair, eye):

        eyes_hot = np.zeros([self.e_len])
        eyes_hot[eye] = 1
        hair_hot = np.zeros([self.h_len])
        hair_hot[hair] = 1
    ...
    def next_data_batch(self, size, neg_sample=False):
        ...
        if self.current + size < self.length:
            img, tags, widx = self.img_feat[self.current:self.current+size], self.tags[self.current:self.current+size], self.w_idx[self.current:self.current+size]
            self.current += size

        else:
        
            img, tags, widx = self.img_feat[-size:], self.tags[-size:], self.w_idx[-size:]
            self.current = 0
            self.size = len(img)
        ...
        #hair eyes 
        for t in tags:
            tag_one_hot.append(self.make_one_hot(t[0], t[1]))
        ...
        #     e = t[1]
        #     while h == t[0]:
        #         h = np.random.choice(self.h_len, 1)

        #     while e == t[1]:
        #         e = np.random.choice(self.e_len, 1)
            
        #     w_tag_one_hot.append(self.make_one_hot(h, e))

        # return img, np.array(tag_one_hot), self.img_feat[widx], np.array(w_tag_one_hot)

        for i in range(self.size):

            id = self.curr_w_idx
            
            while self.tags[id][0] == 0 and self.tags[id][1] == 0:
                self.curr_w_idx +=1
                self.curr_w_idx = self.curr_w_idx % self.length
                id = self.curr_w_idx
            ...
            while h == t[0] and h != 0:
                h = np.random.choice(self.h_len-1, 1) + 1

            while e == t[1] and e != 0:
                e = np.random.choice(self.e_len-1, 1) + 1
            
            w_tag_one_hot.append(self.make_one_hot(h, e))
            w_img.append(self.img_feat[id])

        return img, np.array(tag_one_hot), np.array(w_img), np.array(w_tag_one_hot)
