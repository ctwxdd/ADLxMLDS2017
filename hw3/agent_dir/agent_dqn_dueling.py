...
class Agent_DQN(Agent):
    ...
    def __init__(self, env, args):
        ...
        self.minibatch = 32
        self.esp = 1
        self.model_path = "save/Breakout_dueling.ckpt"
        ...
        self.w2 = tf.get_variable("w2", shape=[7*7*64,512], initializer=tf.contrib.layers.xavier_initializer())

        self.w_adv= tf.get_variable("w_adv", shape=[512, self.num_action], initializer=tf.contrib.layers.xavier_initializer())
        self.b_adv = tf.get_variable("b_adv", shape=[3])

        self.w_v= tf.get_variable("w_v", shape=[512, 1], initializer=tf.contrib.layers.xavier_initializer())
        self.b_v = tf.get_variable("b_v", shape=[1])


        self.py_x = self.build_model(self.input, self.f1, self.f2, self.f3 , self.w1, self.w2,  self.w_adv, self.w_v, self.b_adv, self.b_v)
        ...
        self.w2_r = tf.get_variable("w2_r", shape=[7*7*64,512], initializer=tf.contrib.layers.xavier_initializer())

        self.w_adv_r = tf.get_variable("w_adv_r", shape=[512, self.num_action], initializer=tf.contrib.layers.xavier_initializer())
        self.b_adv_r = tf.get_variable("b_adv_r", shape=[3])
        
        self.w_v_r = tf.get_variable("w_v_r", shape=[512, 1], initializer=tf.contrib.layers.xavier_initializer())
        self.b_v_r = tf.get_variable("b_v_r", shape=[1])

        self.py_x_r =self.build_model(self.input, self.f1_r, self.f2_r,self.f3_r, self.w1_r, self.w2_r, self.w_adv_r, self.w_v_r, self.b_adv_r, self.b_v_r)   
        ...
        if args.test_dqn:
            ...
            self.saver.restore(self.sess, save_path = './save/Breakout_dueling.ckpt')
            print('loading trained model')

    def build_model(self, input1, f1, f2, f3, w1, w2, w_adv, w_v, b_adv, b_v):
        ...
        la = lrelu(tf.matmul(l1, w1))
        lv = lrelu(tf.matmul(l1, w2))

        adv = tf.matmul(la, w_adv) + b_adv
        value = tf.matmul(lv, w_v) + b_v
    
        out = tf.add(value , (adv - tf.reduce_mean(adv, axis=1, keep_dims=True))  )   # Q = V(s) + A(s,a)
        #out = tf.matmul(l2, w2)
        return out
    ...
    def train(self):
        ...
        self.sess.run(self.w_adv_r.assign(self.w_adv))
        self.sess.run(self.w_v_r.assign(self.w_v))

        self.sess.run(self.b_adv_r.assign(self.b_adv))
        self.sess.run(self.b_v_r.assign(self.b_v))
        ...
        while np.mean(self.recent_rlist) < 500 :
            ...
            while not d :
                ...
                if frame > TRAIN_START and frame % ONLINE_UPDATE == 0:
                    ...
                    selected_q_next = Q1[batch_index, max_act4next]

                    #expected_state_action_values = r_stack + DISCOUNT * np.max(Q1, axis=1)#max_act4next
                    expected_state_action_values = r_stack + (1 - d_stack) * DISCOUNT * selected_q_next
